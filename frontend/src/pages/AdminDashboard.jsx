...
export default function AdminDashboard() {
    ...
    const distinctClasses = ['All', ...new Set(eleves.map(e => e.classe).filter(Boolean))]
    const distinctLevels = ['All', ...new Set(eleves.map(e => e.niveau).filter(Boolean))]
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-20 items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xl font-bold text-white tracking-tight">Admin<span className="text-indigo-400">Panel</span></span>
                        </div>
                        <div className="flex items-center gap-6">
                            <div className="text-right hidden sm:block">
                                <p className="text-sm font-bold text-white">{admin.nom}</p>
                                <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Administrateur</p>
                            </div>
                            <button
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] flex items-center gap-6">
                        ...
                        <div>
                            <p className="text-slate-400 text-sm font-bold uppercase tracking-tight">Total Élèves</p>
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] flex items-center gap-6">
                        ...
                        <div>
                            <p className="text-slate-400 text-sm font-bold uppercase tracking-tight">Moyenne Générale</p>
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] flex items-center gap-6">
                        ...
                        <div>
                            <p className="text-slate-400 text-sm font-bold uppercase tracking-tight">Classes</p>
                            <p className="text-3xl font-black text-white">{distinctClasses.length - 1}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="flex flex-wrap gap-4">
                            <div className="flex items-center gap-3 bg-slate-950 border border-slate-800 rounded-2xl px-4 py-2">
                                ...
                                >
                                    <option value="All" className="bg-slate-900">Tout les niveaux</option>
                                    {distinctLevels.filter(l => l !== 'All').map(l => (
                            ...
                            <div className="flex items-center gap-3 bg-slate-950 border border-slate-800 rounded-2xl px-4 py-2">
                                ...
                                >
                                    ...
                                    {distinctClasses.filter(c => c !== 'All').map(c => (
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-[2.5rem] overflow-hidden">
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-800">
                                {filteredEleves.map((eleve) => (
                                    <tr key={eleve.id} className="group hover:bg-slate-800/20 transition-all cursor-pointer">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-slate-800 to-slate-700 flex items-center justify-center text-indigo-400 font-black text-xl border border-slate-700">
                                                    {eleve.nom?.[0]}
                                                </div>
                                                <div>
                                                    <p className="text-white font-bold group-hover:text-indigo-400 transition-colors">{eleve.nom} {eleve.prenom}</p>
                                                    <p className="text-xs text-slate-500">Code: <span className="font-mono text-slate-400">{eleve.code}</span></p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-6 text-center">
                                            <div className="flex flex-col">
                                                <span className="text-slate-300 font-bold">{eleve.niveau || '-'}</span>
                                                <span className="text-xs text-slate-500">{eleve.classe || '-'}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-6 text-center">
                                        ...
                                        <td className="px-6 py-6 text-center">
                                            <span className={`text-xl font-black ${(eleve.moyenne || 0) >= 14 ? 'text-emerald-400' :
                                                    (eleve.moyenne || 0) >= 10 ? 'text-amber-400' : 'text-rose-400'
                                                }`}>
                                                {eleve.moyenne ? eleve.moyenne.toFixed(2) : '-'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-6 text-center">
                                            <span className="text-white font-bold">{eleve.total_briques || '-'}</span>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <button className="p-3 bg-slate-800/50 hover:bg-indigo-600 rounded-xl transition-all group/btn border border-slate-700 hover:border-indigo-500">
                    ...
                    {filteredEleves.length === 0 && (
                        <div className="py-20 text-center">
                            ...
                            <p className="text-slate-500 font-bold text-lg">Aucun élève trouvé.</p>
                        </div>
                    )}
                </div>
            </main>
